...
do
    ...
    if [ $instance != "fronend" ]; then
        Ip=$(aws ec2 describe-instances --instance_id $Instance_Id --query "Reservations[0].Instances[0].PrivateIpAddress" --output text)
    else
        Ip=$(aws ec2 describe-instances --instance_id $Instance_Id --query "Reservations[0].Instances[0].PublicIpAddress" --output text)
    fi
